import { CHANNEL_ETHEREUM_CONNECTION_INJECTED_REQUEST, CHANNEL_ETHEREUM_CONNECTION_RPC_UI, ChannelAppUi, ChannelContentScript, ETHEREUM_PROVIDER_RPC_CALL, ETHEREUM_PROVIDER_RPC_ESTIMATE_GAS, ETHEREUM_PROVIDER_RPC_GET_BALANCE, ETHEREUM_PROVIDER_RPC_GET_BLOCK, ETHEREUM_PROVIDER_RPC_GET_BLOCK_NUMBER, ETHEREUM_PROVIDER_RPC_GET_BLOCK_WITH_TRANSACTIONS, ETHEREUM_PROVIDER_RPC_GET_CODE, ETHEREUM_PROVIDER_RPC_GET_FEE_DATA, ETHEREUM_PROVIDER_RPC_GET_GAS_PRICE, ETHEREUM_PROVIDER_RPC_GET_NETWORK, ETHEREUM_PROVIDER_RPC_GET_STORAGE_AT, ETHEREUM_PROVIDER_RPC_GET_TRANSACTION, ETHEREUM_PROVIDER_RPC_GET_TRANSACTION_COUNT, ETHEREUM_PROVIDER_RPC_GET_TRANSACTION_RECEIPT, ETHEREUM_PROVIDER_RPC_LOOKUP_ADDRESS, ETHEREUM_PROVIDER_RPC_RESOLVE_NAME, ETHEREUM_PROVIDER_RPC_WAIT_FOR_TRANSACTION, getLogger, withContext, withContextPort, } from "@coral-xyz/common";
const logger = getLogger("ethereum-connection");
export function start(cfg, events, b) {
    const ethereumConnection = ChannelAppUi.server(CHANNEL_ETHEREUM_CONNECTION_RPC_UI);
    ethereumConnection.handler(withContextPort(b, events, handle));
    const ethereumConnectionInjected = (() => {
        if (cfg.isMobile)
            return;
        const s = ChannelContentScript.server(CHANNEL_ETHEREUM_CONNECTION_INJECTED_REQUEST);
...
async function handleImpl(ctx, msg) {
    ...
    switch (method) {
        ...
        case ETHEREUM_PROVIDER_RPC_WAIT_FOR_TRANSACTION:
            return await handleWaitForTransaction(ctx, params[0], params[1], params[2]);
        default:
            throw new Error("invalid rpc method");
    }
}
async function handleGetBalance(ctx, publicKey, blockTag) {
    const resp = await ctx.backend.getBalance(publicKey, blockTag);
    return [resp];
}
async function handleGetCode(ctx, address, blockTag) {
    const resp = await ctx.backend.getCode(address, blockTag);
    return [resp];
}
async function handleGetStorageAt(ctx, address, position, blockTag) {
    const resp = await ctx.backend.getStorageAt(address, position, blockTag);
    return [resp];
}
async function handleGetTransactionCount(ctx, address, blockTag) {
    const resp = await ctx.backend.getTransactionCount(address, blockTag);
    return [resp];
}
async function handleGetBlock(ctx, block) {
    const resp = await ctx.backend.getBlock(block);
    return [resp];
}
async function handleGetBlockWithTransactions(ctx, block) {
    const resp = await ctx.backend.getBlockWithTransactions(block);
    return [resp];
}
async function handleLookupAddress(ctx, address) {
    const resp = await ctx.backend.lookupAddress(address);
    return [resp];
}
async function handleResolveName(ctx, name) {
    const resp = await ctx.backend.resolveName(name);
    return [resp];
}
async function handleGetNetwork(ctx) {
...
async function handleGetBlockNumber(ctx) {
    const resp = await ctx.backend.getBlockNumber();
    return [resp];
}
async function handleGetGasPrice(ctx) {
    const resp = await ctx.backend.getGasPrice();
    return [resp];
}
async function handleGetFeeData(ctx) {
    const resp = await ctx.backend.getFeeData();
    return [resp];
}
async function handleCall(ctx, transaction, blockTag) {
    const resp = await ctx.backend.call(transaction, blockTag);
    return [resp];
}
async function handleEstimateGas(ctx, transaction) {
    const resp = await ctx.backend.estimateGas(transaction);
    return [resp];
}
async function handleGetTransaction(ctx, hash) {
    const resp = await ctx.backend.getTransaction(hash);
    return [resp];
}
async function handleGetTransactionReceipt(ctx, hash) {
    const resp = await ctx.backend.getTransactionReceipt(hash);
    return [resp];
}
async function handleWaitForTransaction(ctx, hash, confirms, timeout) {
    const resp = await ctx.backend.waitForTransaction(hash, confirms, timeout);
    return [resp];
}
//# sourceMappingURL=ethereum-connection.js.map