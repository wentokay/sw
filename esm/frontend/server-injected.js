...
import { BACKEND_EVENT, Blockchain, BrowserRuntimeExtension, CHANNEL_ETHEREUM_NOTIFICATION, CHANNEL_ETHEREUM_RPC_REQUEST, CHANNEL_POPUP_RESPONSE, CHANNEL_SOLANA_NOTIFICATION, CHANNEL_SOLANA_RPC_REQUEST, ChannelAppUi, ChannelContentScript, ETHEREUM_RPC_METHOD_CONNECT, ETHEREUM_RPC_METHOD_DISCONNECT, ETHEREUM_RPC_METHOD_SIGN_AND_SEND_TX, ETHEREUM_RPC_METHOD_SIGN_MESSAGE, ETHEREUM_RPC_METHOD_SIGN_TX, ETHEREUM_RPC_METHOD_SWITCH_CHAIN, EthereumChainIds, EthereumConnectionUrl, getLogger, NOTIFICATION_ETHEREUM_ACTIVE_WALLET_UPDATED, NOTIFICATION_ETHEREUM_CHAIN_ID_UPDATED, NOTIFICATION_ETHEREUM_CONNECTED, NOTIFICATION_ETHEREUM_CONNECTION_URL_UPDATED, NOTIFICATION_ETHEREUM_DISCONNECTED, NOTIFICATION_SOLANA_ACTIVE_WALLET_UPDATED, NOTIFICATION_SOLANA_CONNECTED, NOTIFICATION_SOLANA_CONNECTION_URL_UPDATED, NOTIFICATION_SOLANA_DISCONNECTED, openApprovalPopupWindow, openApproveAllTransactionsPopupWindow, openApproveMessagePopupWindow, openApproveTransactionPopupWindow, openLockedApprovalPopupWindow, openLockedPopupWindow, openOnboarding, openPopupWindow, SOLANA_RPC_METHOD_CONNECT, SOLANA_RPC_METHOD_DISCONNECT, SOLANA_RPC_METHOD_OPEN_XNFT, SOLANA_RPC_METHOD_SIGN_ALL_TXS, SOLANA_RPC_METHOD_SIGN_AND_SEND_TX, SOLANA_RPC_METHOD_SIGN_MESSAGE, SOLANA_RPC_METHOD_SIGN_TX, SOLANA_RPC_METHOD_SIMULATE, TAB_XNFT, UiActionRequestManager, withContext, withContextPort, } from "@coral-xyz/common";
...
export function start(cfg, events, b) {
    ...
    const solanaRpcServerInjected = ChannelContentScript.server(CHANNEL_SOLANA_RPC_REQUEST);
    const solanaNotificationsInjected = ChannelContentScript.client(CHANNEL_SOLANA_NOTIFICATION);
    const ethereumRpcServerInjected = ChannelContentScript.server(CHANNEL_ETHEREUM_RPC_REQUEST);
    const ethereumNotificationsInjected = ChannelContentScript.client(CHANNEL_ETHEREUM_NOTIFICATION);
...
async function handle(ctx, req) {
    ...
    //
    if (method !== ETHEREUM_RPC_METHOD_CONNECT &&
    ...
    switch (method) {
        ...
        case SOLANA_RPC_METHOD_SIGN_AND_SEND_TX:
            return await handleSolanaSignAndSendTx(ctx, params[0], params[1], params[2]);
...
async function handleConnect(ctx, blockchain) {
    const origin = ctx.sender.origin;
    if (!origin) {
    ...
    if (keyringStoreState === "locked") {
        if (await ctx.backend.isApprovedOrigin(origin)) {
            ...
            resp = await UiActionRequestManager.requestUiAction((requestId) => {
                return openLockedPopupWindow(origin, getTabTitle(ctx), requestId, blockchain);
            });
            ...
            resp = await UiActionRequestManager.requestUiAction((requestId) => {
                return openLockedApprovalPopupWindow(origin, getTabTitle(ctx), requestId, blockchain);
            });
        ...
        if (await ctx.backend.isApprovedOrigin(origin)) {
            ...
            resp = await UiActionRequestManager.requestUiAction((requestId) => {
                return openApprovalPopupWindow(origin, getTabTitle(ctx), requestId, blockchain);
            });
            didApprove = !resp.windowClosed && resp.result.didApprove;
        }
    }
    ...
    if (didApprove) {
        ...
        if (blockchain === Blockchain.ETHEREUM) {
            const connectionUrl = await ctx.backend.ethereumConnectionUrlRead(user.uuid);
            ...
            const connectionUrl = await ctx.backend.solanaConnectionUrlRead(user.uuid);
...
async function handleDisconnect(ctx, blockchain) {
...
async function handleSolanaSignAndSendTx(ctx, tx, walletAddress, options) {
    if (ctx.sender.origin === undefined) {
        throw new Error("origin is undefined");
    }
    // Get user approval.
    const uiResp = await UiActionRequestManager.requestUiAction((requestId) => {
        return openApproveTransactionPopupWindow(ctx.sender.origin, getTabTitle(ctx), requestId, tx, walletAddress, Blockchain.SOLANA);
    });
    if (uiResp.error) {
    ...
    try {
        ...
        if (didApprove) {
            const sig = await ctx.backend.solanaSignAndSendTx(transaction, walletAddress, options);
            resp = [sig];
        }
    }
    catch (err) {
...
async function handleSolanaSignTx(ctx, tx, walletAddress) {
    if (ctx.sender.origin === undefined) {
        throw new Error("origin is undefined");
    }
    const uiResp = await UiActionRequestManager.requestUiAction((requestId) => {
    ...
    try {
        // Only sign if the user clicked approve.
        if (didApprove) {
            const sig = await ctx.backend.solanaSignTransaction(transaction, walletAddress);
            resp = [sig];
        }
    }
    catch (err) {
...
async function handleSolanaSignAllTxs(ctx, txs, walletAddress) {
    if (ctx.sender.origin === undefined) {
        throw new Error("origin is undefined");
    }
    const uiResp = await UiActionRequestManager.requestUiAction((requestId) => {
        return openApproveAllTransactionsPopupWindow(ctx.sender.origin, getTabTitle(ctx), requestId, txs, walletAddress, Blockchain.SOLANA);
    });
    if (uiResp.error) {
        logger.debug("require ui action error", uiResp);
        BrowserRuntimeExtension.closeWindow(uiResp.window.id);
    ...
    try {
        ...
        if (didApprove) {
            const sigs = await ctx.backend.solanaSignAllTransactions(txs, walletAddress);
            resp = [sigs];
        }
    }
    catch (err) {
...
async function handleSolanaSignMessage(ctx, msg, walletAddress) {
    if (ctx.sender.origin === undefined) {
        throw new Error("origin is undefined");
    }
    const uiResp = await UiActionRequestManager.requestUiAction((requestId) => {
        return openApproveMessagePopupWindow(ctx.sender.origin, getTabTitle(ctx), requestId, msg, walletAddress, Blockchain.SOLANA);
    });
    if (uiResp.error) {
        logger.debug("require ui action error", uiResp);
        BrowserRuntimeExtension.closeWindow(uiResp.window.id);
...
async function handleSolanaOpenXnft(ctx, xnftAddress) {
    const url = `xnft/${xnftAddress}`;
    await ctx.backend.navigationPush(url, TAB_XNFT);
    await openPopupWindow(`popup.html`);
    return ["success"];
}
async function handleEthereumSwitchChain(ctx, chainId) {
    if (ctx.sender.origin === undefined) {
        throw new Error("origin is undefined");
    }
    const chainName = EthereumChainIds[chainId];
    const url = chainName ? EthereumConnectionUrl[chainName] : undefined;
    if (!url) {
        throw new Error("Unsupported Chain: " + chainId);
    }
    const uiResp = await UiActionRequestManager.requestUiAction((requestId) => {
        return openApproveMessagePopupWindow(ctx.sender.origin, getTabTitle(ctx), requestId, `Switch to ${chainName} (${chainId})?`, "walletAddress", Blockchain.ETHEREUM);
    });
    if (uiResp.error) {
        logger.debug("require ui action error", uiResp);
        BrowserRuntimeExtension.closeWindow(uiResp.window.id);
...
async function handleEthereumSignAndSendTx(ctx, tx, walletAddress) {
    if (ctx.sender.origin === undefined) {
        throw new Error("origin is undefined");
    }
    // Get user approval.
    const uiResp = await UiActionRequestManager.requestUiAction((requestId) => {
        return openApproveTransactionPopupWindow(ctx.sender.origin, getTabTitle(ctx), requestId, tx, walletAddress, Blockchain.ETHEREUM);
    });
    if (uiResp.error) {
        logger.debug("require ui action error", uiResp);
        BrowserRuntimeExtension.closeWindow(uiResp.window.id);
    ...
    try {
        // Only sign if the user clicked approve.
        if (didApprove) {
            const sig = await ctx.backend.ethereumSignAndSendTransaction(transaction, walletAddress);
            resp = [sig];
        }
    }
    catch (err) {
...
async function handleEthereumSignTx(ctx, tx, walletAddress) {
    if (ctx.sender.origin === undefined) {
        throw new Error("origin is undefined");
    }
    const uiResp = await UiActionRequestManager.requestUiAction((requestId) => {
...
async function handleEthereumSignMessage(ctx, msg, walletAddress) {
    ...
    const uiResp = await UiActionRequestManager.requestUiAction((requestId) => {
        return openApproveMessagePopupWindow(ctx.sender.origin, getTabTitle(ctx), requestId, msg, walletAddress, Blockchain.ETHEREUM);
    });
    if (uiResp.error) {
        logger.debug("require ui action error", uiResp);
        BrowserRuntimeExtension.closeWindow(uiResp.window.id);
...
async function handlePopupUiResponse(ctx, msg) {
...
function getTabTitle(ctx) {
    var _a, _b;
    return (_b = (_a = ctx.sender.tab) === null || _a === void 0 ? void 0 : _a.title) !== null && _b !== void 0 ? _b : `Xnft from ${ctx.sender.origin}`;
}
//# sourceMappingURL=server-injected.js.map