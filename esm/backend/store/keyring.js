...
export async function getKeyringStore_NO_MIGRATION(password) {
...
export async function setKeyringStore(json, password) {
...
export async function getKeyringCiphertext() {
    return await LocalStorageDb.get(KEY_KEYRING_STORE);
}
async function setKeyringCiphertext(ciphertext) {
    await LocalStorageDb.set(KEY_KEYRING_STORE, ciphertext);
}
export async function doesCiphertextExist() {
