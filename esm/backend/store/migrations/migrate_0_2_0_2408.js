...
import { legacyBip44ChangeIndexed, legacyBip44Indexed, legacySolletIndexed, nextIndicesFromPaths, } from "@coral-xyz/common";
    ...
    for (const [user, userData] of Object.entries(json.users)) {
        ...
        userData.blockchains)) {
            ...
            if (hdKeyring && hdKeyring.accountIndices !== undefined) {
                const derivationPaths = hdKeyring.accountIndices.map((i) => {
                    if (hdKeyring.derivationPath === "bip44") {
                        return legacyBip44Indexed(blockchain, i);
                    }
                    else if (hdKeyring.derivationPath === "bip44-change") {
                        return legacyBip44ChangeIndexed(blockchain, i);
                    }
                    else {
            ...
            if (ledgerKeyring.derivationPaths !== undefined) {
                const walletDescriptors = ledgerKeyring.derivationPaths.map((d) => {
                    let derivationPath;
                    if (d.path === "bip44") {
                        derivationPath = legacyBip44Indexed(blockchain, d.account);
                    }
                    else if (d.path === "bip44-change") {
                        derivationPath = legacyBip44ChangeIndexed(blockchain, d.account);
                    }
                    else {
...
//# sourceMappingURL=migrate_0_2_0_2408.js.map